...
public interface ProductRepository extends BaseRepository<Product, Long>, CustomProductRepository {
    ...
    @Query("select new com.pharmacy.dto.ProductDto(p.id productId, p.name productName, p.genericName, pt.id productTypeId, pt.name productTypeName) from Product p left join fetch ProductType pt on p.productType.id = p.id")
    @Override
